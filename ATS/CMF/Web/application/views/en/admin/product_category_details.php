<div class="main">
	<div class="container">
		<h1>{product_category_details_text} {category_id}
			<?php 
			if($info && $info[$selected_lang]['pcd_name']) 
				echo $comma_text." ".$info[$selected_lang]['pcd_name']
			?>
		</h1>		
		<?php 
			if(!$info) {
			...
			<div class="container">
				<div class="row general-buttons">
					<div class="two columns button sub-primary button-primary"
						onclick="window.open('<?php echo get_customer_product_category_details_link($category_id,$info[$selected_lang]['pc_hash'],'');?>','_blank');"
					>
						{customer_page_text}
					</div>
				</div>
				...
				<br><br>
				<?php echo form_open(get_admin_product_category_details_link($category_id),array("onsubmit"=>"return formSubmit();")); ?>
					...
					<div class="tab-container">
						<ul class="tabs">
							<?php foreach($info as $inf) { ?>
								<li>
									<a href="#pcd_<?php echo $inf['pcd_lang_id'];?>">
										<?php echo $inf['lang']?>
									</a>
								</li>
							<?php } ?>
						...
						<?php foreach($info as $lang=>$cd) {?>
							<div class="tab" id="pcd_<?php echo $cd['pcd_lang_id'];?>">
								<div class="container">
									<div class="row even-odd-bg" >
										...
										<div class="nine columns">
											<input type="text" class="full-width" 
												name="<?php echo $lang;?>[pcd_name]" 
												value="<?php echo $cd['pcd_name']; ?>"
									...
									<div class="row even-odd-bg" >
										...
										<div class="nine columns eng ltr">
											...
											<input type="text" class="six columns eng" 
												name="<?php echo $lang;?>[pcd_url]" 
												value="<?php echo $cd['pcd_url']; ?>"
											/>
										</div>
									</div>
									<div class="row even-odd-bg" >
										...
										<div class="nine columns ">
											<textarea class="full-width" rows="10"
												name="<?php echo $lang;?>[pcd_description]"
											><?php echo $cd['pcd_description']; ?></textarea>
										</div>
									</div>
									<div class="row even-odd-bg" >
										<div class="three columns">
										...
										<div class="nine columns ">
											...
											<div class="tweleve columns">
												<input type="hidden" name="<?php echo $lang;?>[pcd_image]"
												value="<?php echo $cd['pcd_image']; ?>" />
												<?php
													$image=$no_image_url;
													if($cd['pcd_image'])
														$image=$cd['pcd_image'];
									...
									<div class="row even-odd-bg" >
										...
										<div class="nine columns">
											<input type="text" class="full-width" 
												name="<?php echo $lang;?>[pcd_meta_keywords]" 
												value="<?php echo $cd['pcd_meta_keywords']; ?>"
											/>
										</div>
									</div>
									<div class="row even-odd-bg" >
										...
										<div class="nine columns">
											<input type="text" class="full-width" 
												name="<?php echo $lang;?>[pcd_meta_description]" 
												value="<?php echo $cd['pcd_meta_description']; ?>"
											/>
										</div>
									</div>
								</div>
					...
					<div class="row even-odd-bg">
						...
						<div id="parent-category" class="nine columns category-div">
							<input type="hidden" name="pc_parent_id" />
							<?php echo $categories; ?>
						</div>
						<script type="text/javascript">
					...
					<div class="row even-odd-bg">
						...
						<div class="nine columns">
							<input type="text" class="ltr en full-width" name="pc_hash" 
								value="<?php echo $info[$selected_lang]['pc_hash']?>"
							/>
						</div>
					</div>
					<div class="row even-odd-bg">
						...
						<div class="nine columns">
							<input type="checkbox" class="graphical" name="pc_show_in_list" 
								<?php if($info[$selected_lang]['pc_show_in_list']) echo 'checked';?>
							/>
						</div>
					</div>
					<div class="row even-odd-bg">
						...
						<div class="nine columns">
							<input type="checkbox" class="graphical" name="pc_is_hidden" 
								<?php if($info[$selected_lang]['pc_is_hidden']) echo 'checked';?>
							/>
						</div>
					</div>
					<script type="text/javascript">
						...
						function fileSelected(path)
						{
							...
							$("input[name='"+activeLang+"[pcd_image]']").val(path);
						...
						function deleteImage(lang)
						{
							if(typeof(lastImages[lang])==="undefined" || lastImages[lang]=="")
							{
								...
								$("input[name='"+lang+"[pcd_image]']").val("");
								$("#img-"+lang).prop("src","{no_image_url}");
							}
							else
							{
								$("input[name='"+lang+"[pcd_image]']").val(lastImages[lang]);
					...
					<script type="text/javascript">
						function setUrl(el,lang)
						{
							...
							$("input[name='"+lang+"[pcd_url]']").val(val);
						}
					</script>
				</form>
	              	...
	              	function deleteCategory()
						...
						<?php
							foreach($all_langs as $lang => $value)
								echo "\n".'tmTextAreas.push("textarea[name=\''.$lang.'[pcd_description]\']");';
						?>
